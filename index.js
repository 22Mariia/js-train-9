...
function findElementGreaterThan(arr, num) {
  if (!Array.isArray(arr)) {
    ...
    const findArr = arr.find(elem => elem > num );
    if(arr.some(elem => elem > num)){
     return findArr;
    }
    else{
...
function findIndexByProperty(arr, name) {
   if (!Array.isArray(arr)) {
    return -1;}
    const indexArr = arr.findIndex(({num, elem}) => elem === name);
    if(({elem}) => elem === name){
      return indexArr; 
...
function findLastEvenNumber(arr) {
   if (!Array.isArray(arr)) {
    ...
    const s = arr.findLast((element) => element % 2 ===0);
...
function getPositiveArrayValues(arr) {
  ...
  const filterResult = result.filter(result => result > 0);
...
function removeAndDouble(arr, startIdx) {
  if (!Array.isArray(arr)) {
    return 'Вхідний параметр має бути масивом';}
  // Перевіряємо, чи вхідний параметр є масивом, якщо ні повертаємо 'Вхідний параметр має бути масивом'
  const splaceArr = arr.splice(startIdx);
...
function sumAndPrint(arr) {
  if (!Array.isArray(arr)) {
    return 'Вхідний параметр має бути масивом';}
  // Перевіряємо, чи вхідний параметр є масивом, якщо ні повертаємо 'Вхідний параметр має бути масивом'
  const reducaArr = arr.reduce((accumulator, currentValue) => accumulator + currentValue,)
  // За допомогою методу reduce() обчислюємо суму елементів масиву.
  const foreachArr = arr.forEach(element => console.log(element));
...
function flattenSquareAndFilter(arr, threshold) {
  ...
  const result = flatArr.map(x => x**2);
  // За допомогою методу map() підносимо кожен елемент масиву до квадрату.
  const filterResult = result.filter(result => result > threshold);
...
function wordLengths(arr) {
  if (!Array.isArray(arr)) {
    return 'Вхідний параметр має бути масивом';}
  // Перевіряємо, чи вхідний параметр є масивом, якщо ні, повертаємо 'Вхідний параметр має бути масивом'
  const mapArr = arr.map( element =>  element.length )
...
function insertElementAt(arr, index, element) {
  ...
  // Перевіряємо, чи вхідний параметр є масивом, якщо ні повертаємо 'Вхідний параметр має бути масивом'
   const spliceArr = arr.splice(index, 0, element);
...
function expandArrayByN(arr, n) {
  ...
  const flatMapArr = arr.flatMap((num) => n?[num, num, num]:1);
...
function findLongestWord(arr) {
  ...
  const reduceArr = arr.reduce((allfruit, fruit) => allfruit = fruit.length  , 0);
  const fruitArr = arr.filter((fruit) => fruit.length === reduceArr);
...
function findDuplicateElements(arr) {
  ...
  const filterArr = arr.filter(function(number){
    return number === number;
  });
  // Використовуємо метод filter() для вибірки лише дубльованих елементів 
 const indexOfFirst = arr.indexOf(filterArr);
 return indexOfFirst;
...
function calculateTotalPrice(arr) {
  ...
  // const reduseArr = arr.reduce(( num, product, prodsctIndex, array) => {
  //   console.log(num, product.price);
...
function findLastNegativeNumberIndex(arr) {
  ...
  // Перевіряємо, чи вхідний параметр є масивом, якщо ні, повертаємо -1
const lastIndexArr = arr.findLastIndex( (elem) => elem < 0);
